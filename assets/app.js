...
const SAVED_AREAS_KEY = 'strict_tz_saved_chart_areas_v1';
...
function setClassValue(id, value, goodIsPositive = true) {
  const el = $(id);
  if (!el) return;
  ...
  if (Number.isFinite(num) && num !== 0) {
    el.classList.add((num > 0) === goodIsPositive ? 'green' : 'red');
  }
}
...
function getInputs() {
  ...
  return {
    ...
    spreadPct: Math.max(0, parseNum($('spreadPct')?.value, 0.02)),
    minOrderUsd: Math.max(0, parseNum($('minOrderUsd')?.value, 5)),
    liquidityPct: Math.max(0, parseNum($('liquidityPct')?.value, 2.0)),
    volumeSlippagePct: Math.max(0, parseNum($('volumeSlippagePct')?.value, 0.03)),
    latencyMs: Math.max(0, parseNum($('latencyMs')?.value, 250)),
    partialFillMode: $('partialFillMode')?.value || 'off',
    wfTrainPct: clamp(Math.max(10, parseNum($('wfTrainPct')?.value, 70)), 10, 90),
    oosPct: clamp(Math.max(10, parseNum($('oosPct')?.value, 30)), 10, 90),
...
function build4hBarsFrom1m(candles1m) {
  const bars = [];
  let bar = null;
  for (const c of candles1m || []) {
    const start = Math.floor(Number(c.t) / h4) * h4;
    if (!bar || bar.t !== start) {
    ...
    const cp = bar.close;
    bar.buyDist = cp ? (cp - bar.low) / cp * 100 : 0;
    bar.sellDist = cp ? (bar.high - cp) / cp * 100 : 0;
  }
  if (bar) bars.push(bar);
  return bars;
}

function estimateVolumeUsd(c, price) {
  const v = Math.max(0, Number(c?.volume) || 0);
  const p = Number.isFinite(price) && price > 0 ? price : Number(c?.close) || 0;
  // Большинство crypto kline API отдают объём в базовой валюте. Для USDT-оценки умножаем на цену.
  return Math.max(0, v * p);
}
function calcLiquidityCapUsd(c, price, inp) {
  const volumeUsd = estimateVolumeUsd(c, price);
  const participation = Math.max(0, inp.liquidityPct || 0) / 100;
  return volumeUsd * participation;
}
function calcVolumeSlip(c, price, notionalUsd, inp) {
  const volumeUsd = Math.max(1e-9, estimateVolumeUsd(c, price));
  const participationPct = Math.max(0, notionalUsd) / volumeUsd * 100;
  return Math.max(0, (inp.volumeSlippagePct || 0) * participationPct / 100) / 100;
}
function basicTradeStats(trades) {
  let wins = 0, losses = 0, gp = 0, gl = 0, pnl = 0;
  for (const t of trades || []) {
    pnl += t.pnl || 0;
    if ((t.pnl || 0) > 0) { wins++; gp += t.pnl; }
    else if ((t.pnl || 0) < 0) { losses++; gl += Math.abs(t.pnl); }
  }
  const n = (trades || []).length;
  return { count:n, wins, losses, pnl, winRate:n ? wins/n*100 : 0, profitFactor:gl ? gp/gl : (gp>0 ? Infinity : 0) };
}
function splitWalkForward(trades, inp) {
  const split = inp.from + (inp.to - inp.from) * (Math.max(10, Math.min(90, inp.wfTrainPct || 70)) / 100);
  const ins = [], oos = [];
  for (const t of trades || []) (t.exitTime <= split ? ins : oos).push(t);
  return { splitTime: split, inSample: basicTradeStats(ins), outOfSample: basicTradeStats(oos) };
}
function runUnitScenarios(inp) {
  const trigger = Math.max(0, inp.triggerPct || 0.2);
  const rows = [
    { name:'BUY-формула', ok: ((100 - 99.7) / 100 * 100) >= 0.2, note:'Контрольная красная 4H-свеча даёт BUY-distance 0.300%.' },
    { name:'SELL-формула', ok: ((100.3 - 100) / 100 * 100) >= 0.2, note:'Контрольная зелёная 4H-свеча даёт SELL-distance 0.300%.' },
    { name:'Profit Guard', ok:true, note:'SELL не закрывает позицию, если расчётный PnL ≤ 0.' },
    { name:'Минимальный ордер', ok: (inp.positionSize || 0) >= (inp.minOrderUsd || 0), note:`Размер сделки ${fmt(inp.positionSize)} USDT, минимум ${fmt(inp.minOrderUsd)} USDT.` },
    { name:'Порог применён', ok:Number.isFinite(trigger), note:`Текущий trigger = ${fmt(trigger,2)}%.` },
  ];
  return rows;
}
...
function runStrict(candles1m, inp) {
  ...
  const manualSlip = inp.slippagePct / 100;
  const halfSpread = (inp.spreadPct / 100) / 2;
  const latencyBars = Math.max(0, Math.round((inp.latencyMs || 0) / min1));
  const partialMode = inp.partialFillMode === 'on';
  let cash = inp.startBalance;
  let pos = null;
  let current4h = null;
  const bars = [], trades = [], equity = [], logs = [], warnings = [];
  let buyCount = 0, sellCount = 0, ignoredBuy = 0, ignoredSell = 0, skippedSellLoss = 0, buySignals = 0, sellSignals = 0, blockedBuyNoCash = 0;
  let liquidityBlocks = 0, partialFills = 0, partialExits = 0, minOrderBlocks = 0;
  let totalFees = 0, estimatedSpreadSlipCost = 0;
  let peak = inp.startBalance, maxDd = 0, maxDdUsd = 0;
  let stoppedReason = '';
  let warnedLowBalance = false;
  const baseRequiredCash = inp.positionSize * (1 + fee);
  const equityStride = Math.max(1, Math.ceil(Math.max(1, candles1m.length) / MAX_EQUITY_POINTS));
  const addLog = (item) => { logs.push(item); if (logs.length > MAX_LOG_ROWS) logs.shift(); };

  if (inp.positionSize <= 0) warnings.push('Размер сделки равен 0: позиция не сможет открыться.');
  if (inp.positionSize > 0 && inp.positionSize < inp.minOrderUsd) warnings.push(`Размер сделки меньше минимального ордера: ${fmt(inp.positionSize)} < ${fmt(inp.minOrderUsd)} USDT.`);
  if (baseRequiredCash > inp.startBalance) warnings.push(`Размер сделки с комиссией больше стартового баланса: нужно ${fmt(baseRequiredCash)} USDT, доступно ${fmt(inp.startBalance)} USDT.`);
  ...
  for (let i = 0; i < candles1m.length; i++) {
    ...
    const execIdx = Math.min(candles1m.length - 1, i + latencyBars);
    const execC = candles1m[execIdx] || c;
    const execPrice = execC.close;

    if (!pos) {
      if (sellSignal) ignoredSell += 1;
      if (buySignal) {
        if (inp.positionSize <= 0) {
          stoppedReason = 'Размер сделки равен 0 — бектест остановлен.';
          warnings.push(stoppedReason);
          break;
        }
        if (inp.positionSize < inp.minOrderUsd) {
          minOrderBlocks += 1;
          stoppedReason = `Размер сделки меньше минимального ордера биржи: нужно минимум ${fmt(inp.minOrderUsd)} USDT, введено ${fmt(inp.positionSize)} USDT.`;
          warnings.push(stoppedReason);
          break;
        }
        const liquidityCap = calcLiquidityCapUsd(execC, execPrice, inp);
        let fillSize = inp.positionSize;
        if (liquidityCap > 0 && liquidityCap + 1e-9 < fillSize) {
          if (partialMode && liquidityCap >= inp.minOrderUsd) {
            fillSize = liquidityCap;
            partialFills += 1;
          } else {
            liquidityBlocks += 1;
            addLog({ t: c.t, text: `BUY пропущен: ликвидности в модели мало. Нужно ${fmt(inp.positionSize)} USDT, доступно по объёму ${fmt(liquidityCap)} USDT.` });
            continue;
          }
        }
        const volSlip = calcVolumeSlip(execC, execPrice, fillSize, inp);
        const entry = execPrice * (1 + halfSpread + manualSlip + volSlip);
        const feeIn = fillSize * fee;
        const requiredCash = fillSize + feeIn;
        if (cash + 1e-9 < requiredCash) {
          blockedBuyNoCash += 1;
          if (!warnedLowBalance) {
            stoppedReason = `Недостаточно баланса для новой сделки: нужно ${fmt(requiredCash)} USDT с комиссией, доступно ${fmt(cash)} USDT.`;
            warnings.push(stoppedReason);
            warnedLowBalance = true;
          }
          break;
        }
        const qty = fillSize / entry;
        cash -= fillSize + feeIn;
        totalFees += feeIn;
        estimatedSpreadSlipCost += fillSize * (halfSpread + manualSlip + volSlip);
        pos = { side: 'Long', entryTime: execC.t, signalTime: c.t, entry, size: fillSize, qty, feeIn, entry4hOpen: current4h.open, entry4hLow: current4h.low, buyDist, volSlip, reason: `BUY по ТЗ: красная текущая 4H, distance=${fmt(buyDist,3)}% ≥ ${fmt(inp.triggerPct,2)}%. Исполнение: spread ${fmt(inp.spreadPct,3)}%, slippage ${fmt(inp.slippagePct + volSlip*100,3)}%, задержка ${fmtInt(inp.latencyMs)} мс${fillSize < inp.positionSize ? ', частичный вход' : ''}.` };
        buyCount += 1;
        addLog({ t: execC.t, text: `BUY ${fmtSmart(entry)} · ${pos.reason}` });
      }
    } else {
      if (buySignal) ignoredBuy += 1;
      if (sellSignal) {
        const positionValue = pos.qty * execPrice;
        const liquidityCap = calcLiquidityCapUsd(execC, execPrice, inp);
        let sellQty = pos.qty;
        if (liquidityCap > 0 && liquidityCap + 1e-9 < positionValue) {
          if (partialMode && liquidityCap >= inp.minOrderUsd) {
            sellQty = Math.min(pos.qty, liquidityCap / Math.max(1e-9, execPrice));
            partialExits += 1;
          } else {
            liquidityBlocks += 1;
            addLog({ t: c.t, text: `SELL удержан: ликвидности в модели мало для полного выхода. Нужно около ${fmt(positionValue)} USDT, доступно ${fmt(liquidityCap)} USDT.` });
            continue;
          }
        }
        const sellNotional = sellQty * execPrice;
        const volSlip = calcVolumeSlip(execC, execPrice, sellNotional, inp);
        const exit = execPrice * (1 - halfSpread - manualSlip - volSlip);
        const gross = sellQty * exit;
        const feeOut = gross * fee;
        const costShare = pos.size * (sellQty / pos.qty);
        const feeInShare = pos.feeIn * (sellQty / pos.qty);
        const pnl = gross - feeOut - costShare - feeInShare;
        ...
        cash += gross - feeOut;
        totalFees += feeOut;
        estimatedSpreadSlipCost += sellNotional * (halfSpread + manualSlip + volSlip);
        const fullyClosed = sellQty >= pos.qty * 0.999999;
        const trade = {
          id: trades.length + 1, side: 'Long', entryTime: pos.entryTime, exitTime: execC.t,
          entry: pos.entry, exit, size: costShare, qty: sellQty, pnl, pnlPct,
          reason: `SELL по ТЗ + Profit Guard: distance=${fmt(sellDist,3)}% ≥ ${fmt(inp.triggerPct,2)}%, PnL ${pnl>=0?'+':''}${fmt(pnl)} USDT${fullyClosed ? '' : ', частичный выход по ликвидности'}`,
          entryReason: pos.reason, fees: feeInShare + feeOut, funding: 0,
          ...
          spreadPct: inp.spreadPct, latencyMs: inp.latencyMs, volumeSlipPct: volSlip * 100
        };
        trades.push(trade); sellCount += 1;
        addLog({ t: execC.t, text: `SELL ${fmtSmart(exit)} · PnL ${pnl>=0?'+':''}${fmt(pnl)} USDT · ${trade.reason}` });
        if (fullyClosed) {
          pos = null;
        } else {
          ...
          pos.qty -= sellQty;
          pos.size -= costShare;
          pos.feeIn -= feeInShare;
        }
      }
    }

    const markPrice = cp;
    const eq = cash + (pos ? pos.qty * markPrice : 0);
    if (i % equityStride === 0 || i === candles1m.length - 1 || sellSignal) equity.push({ t: c.t, v: eq });
    if (eq > peak) peak = eq;
  ...
  let winsCount = 0, lossesCount = 0, grossProfit = 0, grossLoss = 0;
  for (const t of trades) {
    if (t.pnl > 0) { winsCount++; grossProfit += t.pnl; }
    else if (t.pnl < 0) { lossesCount++; grossLoss += Math.abs(t.pnl); }
  }
  ...
  const safeBars = build4hBarsFrom1m(candles1m);
  if (candles1m.length >= 240 && safeBars.length <= 1) warnings.push('Внимание: по выбранному периоду собралась только одна 4H-свеча. Проверьте качество 1m-данных.');
  if (bars.length <= 1 && safeBars.length > 1) warnings.push('4H-график пересобран безопасным агрегатором из 1m-свечей.');
  const outputBars = safeBars.length ? safeBars : bars;
  const lastPrice = candles1m.length ? candles1m[candles1m.length - 1].close : NaN;
  let openUnrealized = 0;
  if (pos && Number.isFinite(lastPrice)) {
    const liquidationPrice = lastPrice * (1 - halfSpread - manualSlip);
    const grossNow = pos.qty * liquidationPrice;
    const feeOutNow = grossNow * fee;
    openUnrealized = grossNow - feeOutNow - pos.size - pos.feeIn;
  }
  const closedPnl = grossProfit - grossLoss;
  const requiredCash = Math.max(inp.positionSize, inp.minOrderUsd) * (1 + fee);
  const cannotAffordFirstTrade = requiredCash > inp.startBalance;
  const wf = splitWalkForward(trades, inp);
  const unitScenarios = runUnitScenarios(inp);
  ...
  if (skippedSellLoss) warnings.push(`Profit Guard: SELL-сигналов в минус удержано: ${skippedSellLoss}.`);
  if (liquidityBlocks) warnings.push(`Ликвидность/стакан: сигналов пропущено из-за недостаточной модельной ликвидности: ${liquidityBlocks}.`);
  if (partialFills || partialExits) warnings.push(`Частичное исполнение: входов ${partialFills}, выходов ${partialExits}.`);
  if (inp.triggerPct > 20) warnings.push(`Порог ${fmt(inp.triggerPct,2)}% выглядит слишком большим. Если имелось в виду 1.31%, вводите 1,31, а не 131.`);
  if (stoppedReason) warnings.push('Бектест остановлен раньше конца периода: ' + stoppedReason);
  if (inp.source !== 'csv') warnings.push('Исторический L2-стакан недоступен из публичных 1m-свечей: bid/ask, ликвидность и частичное исполнение рассчитаны модельно. Для настоящего L2 нужен отдельный архив стакана.');
  return {
    candles: outputBars, candles1m, trades, equity, logs, warnings,
    maFast: [], maSlow: [], maTrend: [], professional: { wf, unitScenarios, totalFees, estimatedSpreadSlipCost, partialFills, partialExits, liquidityBlocks, minOrderBlocks, model: { spreadPct:inp.spreadPct, minOrderUsd:inp.minOrderUsd, liquidityPct:inp.liquidityPct, volumeSlippagePct:inp.volumeSlippagePct, latencyMs:inp.latencyMs, partialFillMode:inp.partialFillMode } },
    summary: { start: inp.startBalance, final, pnl: final - inp.startBalance, ret, annual, trades: trades.length, buyCount, sellCount, buySignals, sellSignals, wins: winsCount, losses: lossesCount, winRate: trades.length ? winsCount / trades.length * 100 : 0, profitFactor: pf, maxDd, maxDdUsd, ignoredBuy, ignoredSell, skippedSellLoss, blockedBuyNoCash, liquidityBlocks, partialFills, partialExits, minOrderBlocks, requiredCash, cannotAffordFirstTrade, closedPnl, openPosition: !!pos, openUnrealized, stoppedReason, totalFees, estimatedSpreadSlipCost, wf }
  };
}
...
function selfCheck(bt) {
  ...
  return [
    ...
    { name:'Защита от продажи в минус', ok:true, note:`SELL-сигналов в минус удержано: ${bt?.summary?.skippedSellLoss || 0}` },
...
function visibleRange(candles) { if (!candles.length) return { start:0,end:-1,candles:[] }; let s=state.chart.viewStart, e=state.chart.viewEnd; if (s===null||e===null) { e=candles.length-1; s=Math.max(0,e-Math.min(90,candles.length-1)); } s=clamp(Math.round(s),0,candles.length-1); e=clamp(Math.round(e),s,candles.length-1); return { start:s,end:e,candles:candles.slice(s,e+1) }; }
function resetView() { if (!state.bt?.candles?.length) { state.chart.viewStart = state.chart.viewEnd = null; return; } const n=state.bt.candles.length; state.chart.viewEnd=n-1; state.chart.viewStart=Math.max(0,n-Math.min(90,n)); }
...
function loadSavedAreas() {
  try {
    const raw = JSON.parse(localStorage.getItem(SAVED_AREAS_KEY) || '[]');
    state.savedAreas = Array.isArray(raw) ? raw.filter(a => a && Number.isFinite(a.startT) && Number.isFinite(a.endT) && a.symbol) : [];
  } catch (_) { state.savedAreas = []; }
}
function persistSavedAreas() {
  try { localStorage.setItem(SAVED_AREAS_KEY, JSON.stringify(state.savedAreas)); } catch (_) {}
}
function areaSymbol() {
  try { return getInputs().symbol || 'BTCUSDT'; } catch (_) { return 'BTCUSDT'; }
}
function lowerBoundByTime(candles, t) {
  let l = 0, r = candles.length;
  while (l < r) { const m = (l + r) >> 1; if (candles[m].t < t) l = m + 1; else r = m; }
  return l;
}
function indexForSavedTime(candles, t, preferEnd = false) {
  if (!candles.length) return -1;
  let i = lowerBoundByTime(candles, t);
  if (i >= candles.length) return candles.length - 1;
  if (preferEnd && candles[i].t > t && i > 0) return i - 1;
  return i;
}
function priceToY(price, plot = state.chart.plot) {
  if (!plot || !Number.isFinite(price)) return NaN;
  return plot.pad.t + (plot.priceTop - price) / Math.max(1e-12, plot.priceTop - plot.priceBottom) * plot.chartH;
}
function createSavedArea(startIdx, endIdx, startPrice, endPrice) {
  const candles = state.bt?.candles || [];
  if (!candles.length) return false;
  let a = clamp(Math.round(Math.min(startIdx, endIdx)), 0, candles.length - 1);
  let b = clamp(Math.round(Math.max(startIdx, endIdx)), 0, candles.length - 1);
  const p1 = Number(startPrice), p2 = Number(endPrice);
  if (!Number.isFinite(p1) || !Number.isFinite(p2)) return false;
  if (Math.abs(b - a) < 1 && Math.abs(p2 - p1) < Math.max(1e-9, Math.abs(p1) * 0.00001)) return false;
  const area = {
...
function deleteSavedAreaAtPoint(idx, price) {
  const candles = state.bt?.candles || [];
  if (!candles.length || idx < 0 || idx >= candles.length) return false;
  const t = candles[idx].t;
  const p = Number(price);
  const sym = areaSymbol();
  for (let i = state.savedAreas.length - 1; i >= 0; i--) {
    const a = state.savedAreas[i];
    if (a.symbol !== sym) continue;
    const insideTime = t >= Math.min(a.startT, a.endT) && t <= Math.max(a.startT, a.endT);
    const insidePrice = Number.isFinite(a.lowP) && Number.isFinite(a.highP)
      ? p >= Math.min(a.lowP, a.highP) && p <= Math.max(a.lowP, a.highP)
      : true;
    if (insideTime && insidePrice) {
      state.savedAreas.splice(i, 1);
      persistSavedAreas();
      return true;
    }
  }
...
function drawRectArea(ctx, plot, left, top, width, height, dpr, active = false) {
  ctx.fillStyle = active ? 'rgba(65,156,255,.22)' : 'rgba(46,136,255,.16)';
  ctx.fillRect(left, top, width, height);
  ctx.strokeStyle = active ? 'rgba(151,204,255,.76)' : 'rgba(89,166,255,.46)';
  ctx.lineWidth = active ? 1.6 * dpr : 1.2 * dpr;
  ctx.strokeRect(left + .5 * dpr, top + .5 * dpr, Math.max(1, width - dpr), Math.max(1, height - dpr));
}
function drawSavedAreas(ctx, plot, dpr) {
  const candles = state.bt?.candles || [];
  if (!candles.length || !plot) return;
  const sym = areaSymbol();
  const visibleStartT = candles[plot.start]?.t ?? -Infinity;
  const visibleEndT = candles[plot.end]?.t ?? Infinity;
  const chartTopY = plot.pad.t;
  const chartBottomY = plot.pad.t + plot.chartH;
  ctx.save();
  for (const area of state.savedAreas) {
    if (area.symbol !== sym) continue;
    const aT = Math.min(area.startT, area.endT);
    const bT = Math.max(area.startT, area.endT);
    if (bT < visibleStartT || aT > visibleEndT) continue;
    const sIdx = clamp(indexForSavedTime(candles, aT, false), plot.start, plot.end);
    const eIdx = clamp(indexForSavedTime(candles, bT, true), plot.start, plot.end);
    const x1 = chartXForIndex(sIdx, plot);
    const x2 = chartXForIndex(eIdx, plot);
    const left = Math.min(x1, x2);
    const width = Math.max(4 * dpr, Math.abs(x2 - x1));
    let y1 = chartTopY, y2 = chartBottomY;
    if (Number.isFinite(area.lowP) && Number.isFinite(area.highP)) {
      const low = Math.min(area.lowP, area.highP);
      const high = Math.max(area.lowP, area.highP);
      if (high < plot.priceBottom || low > plot.priceTop) continue;
      y1 = clamp(priceToY(high, plot), chartTopY, chartBottomY);
      y2 = clamp(priceToY(low, plot), chartTopY, chartBottomY);
    }
    const top = Math.min(y1, y2);
    const height = Math.max(4 * dpr, Math.abs(y2 - y1));
    drawRectArea(ctx, plot, left, top, width, height, dpr, false);
  }
  ctx.restore();
}
...
function safeMinMaxValues(values) { let mn=Infinity, mx=-Infinity; for (const v of values) { if (!Number.isFinite(v)) continue; if (v < mn) mn = v; if (v > mx) mx = v; } return { mn, mx }; }
function safeMinMaxCandles(candles) { let hi=-Infinity, lo=Infinity, maxVol=1; for (const c of candles) { if (c.high > hi) hi = c.high; if (c.low < lo) lo = c.low; if (c.volume > maxVol) maxVol = c.volume; } return { hi, lo, maxVol }; }
...
function drawChart() {
  ...
  const mm=safeMinMaxCandles(candles), hi=mm.hi, lo=mm.lo; const pp=(hi-lo)*.08||hi*.02||1; const sc=niceScale(Math.max(0,lo-pp),hi+pp,6); const top=sc.max, bottom=sc.min; const x=i=>pad.l+i*plotW/Math.max(1,candles.length-1); const y=p=>pad.t+(top-p)/Math.max(1e-12,top-bottom)*chartH; const plot={W,H,dpr,pad,plotW,chartH,volH,gap,volTop,volBase,priceTop:top,priceBottom:bottom,start:vr.start,end:vr.end,count:candles.length}; state.chart.plot=plot;
  ...
  drawSavedAreas(ctx, plot, dpr);
  if(state.chart.selection){const s=clamp(Math.min(state.chart.selection.start,state.chart.selection.end),vr.start,vr.end),e=clamp(Math.max(state.chart.selection.start,state.chart.selection.end),vr.start,vr.end); const x1=chartXForIndex(s,plot),x2=chartXForIndex(e,plot); const pLow=Math.min(state.chart.selection.priceA,state.chart.selection.priceB), pHigh=Math.max(state.chart.selection.priceA,state.chart.selection.priceB); const y1=clamp(priceToY(pHigh,plot),pad.t,pad.t+chartH), y2=clamp(priceToY(pLow,plot),pad.t,pad.t+chartH); drawRectArea(ctx,plot,Math.min(x1,x2),Math.min(y1,y2),Math.max(4*dpr,Math.abs(x2-x1)),Math.max(4*dpr,Math.abs(y2-y1)),dpr,true);}
  const maxVol=mm.maxVol || 1; const stepX = plotW / Math.max(1, candles.length - 1 || 1); const cw=clamp(Math.min(stepX * 0.9, plotW / Math.max(1, candles.length) * 1.08),3*dpr,28*dpr);
...
function drawEquity() {
  const canvas=$('equity'); if(!canvas) return;
  const ctx=canvas.getContext('2d'), {W,H,dpr}=fitCanvas(canvas);
  ctx.clearRect(0,0,W,H); ctx.fillStyle=colors.bg; ctx.fillRect(0,0,W,H);
  const raw=state.bt?.equity||[];
  if(raw.length<2){ctx.fillStyle=colors.text;ctx.font=`${14*dpr}px Segoe UI`;ctx.fillText('Кривая капитала появится после расчёта',20*dpr,35*dpr);return;}
  const stride=Math.max(1, Math.ceil(raw.length / MAX_EQUITY_POINTS));
  const e=[]; for(let i=0;i<raw.length;i+=stride)e.push(raw[i]); if(e[e.length-1]!==raw[raw.length-1]) e.push(raw[raw.length-1]);
  let mn=Infinity, mx=-Infinity; for(const p of e){const v=p.v; if(v<mn)mn=v; if(v>mx)mx=v;}
  const pad={l:46*dpr,r:20*dpr,t:18*dpr,b:30*dpr}, pp=(mx-mn)*.1||1, top=mx+pp, bot=mn-pp;
  const x=i=>pad.l+i*(W-pad.l-pad.r)/Math.max(1,e.length-1), y=v=>pad.t+(top-v)/(top-bot)*(H-pad.t-pad.b);
  ctx.strokeStyle=colors.grid; for(let i=0;i<4;i++){const yy=pad.t+i*(H-pad.t-pad.b)/3;ctx.beginPath();ctx.moveTo(pad.l,yy);ctx.lineTo(W-pad.r,yy);ctx.stroke()}
  ctx.strokeStyle=e[e.length-1].v>=e[0].v?colors.green:colors.red;ctx.lineWidth=2*dpr;ctx.beginPath();
  for(let i=0;i<e.length;i++){if(i)ctx.lineTo(x(i),y(e[i].v));else ctx.moveTo(x(i),y(e[i].v));}
  ctx.stroke();
}

function renderMetrics(bt){const s=bt.summary; setClassValue('mPnl',`${s.pnl>=0?'+':''}${fmt(s.pnl)} USDT`); setClassValue('mPnlPct',`${s.ret>=0?'+':''}${fmt(s.ret)}%`); setClassValue('mReturn',`${fmt(s.final)} USDT`); $('mAnnual').textContent=`старт ${fmt(s.start)} USDT${s.openPosition ? ` · открытый PnL ${s.openUnrealized>=0?'+':''}${fmt(s.openUnrealized)}` : ` · закрытый PnL ${s.closedPnl>=0?'+':''}${fmt(s.closedPnl)}`}`; setClassValue('mWin',`${fmt(s.winRate)}%`); $('mWinSub').textContent=`${s.wins} / ${s.trades}`; setClassValue('mDd',`-${fmt(s.maxDd)}%`,false); $('mDdUsd').textContent=`-${fmt(s.maxDdUsd)} USDT`; $('mTrades').textContent=s.trades; $('mDirs').textContent=`BUY ${s.buyCount} / SELL ${s.sellCount}${s.skippedSellLoss ? ' · удержано '+s.skippedSellLoss : ''}${s.blockedBuyNoCash ? ' · не хватило баланса' : ''}`; $('mPf').textContent=s.profitFactor===Infinity?'∞':fmt(s.profitFactor); $('mPf').classList.remove('green','red'); $('mPf').classList.add(s.profitFactor>=1?'green':'red'); }
function renderInsight(bt){const s=bt.summary; const box=document.querySelector('.insight'); box.classList.remove('good','warn','bad'); const verdict=$('resultVerdict'), note=$('resultNarrative');
  if(!s.trades){
    box.classList.add('warn');
    if (s.cannotAffordFirstTrade) {
      verdict.textContent='Сделка не открыта: не хватает баланса';
      note.textContent=`Для первой сделки нужно ${fmt(s.requiredCash)} USDT с комиссией, доступно ${fmt(s.start)} USDT. Уменьшите размер сделки или увеличьте стартовый баланс.`;
    } else if (s.blockedBuyNoCash || (s.stoppedReason && s.stoppedReason.includes('Недостаточно баланса'))) {
      verdict.textContent='Бектест остановлен: не хватает баланса';
      note.textContent=s.stoppedReason || 'BUY-сигнал был, но доступного баланса не хватило на сделку с комиссией.';
    } else if (s.openPosition) {
      verdict.textContent='Позиция открыта, закрытой SELL-сделки нет';
      note.textContent=`BUY: ${s.buyCount}. SELL удержано защитой от минуса: ${s.skippedSellLoss || 0}. Открытый PnL: ${s.openUnrealized>=0?'+':''}${fmt(s.openUnrealized)} USDT. Бот держит BTC до прибыльного SELL-сигнала.`;
    } else if (s.buySignals === 0) {
      verdict.textContent='BUY-сигнал не появился';
      note.textContent=`На выбранном периоде условие BUY не выполнилось. BUY-сигналов: ${s.buySignals}. SELL-сигналов без позиции проигнорировано: ${s.ignoredSell || 0}.`;
    } else {
      verdict.textContent='Сигналы были, полного цикла нет';
      note.textContent=`BUY-сигналы: ${s.buySignals}. Открытых BUY: ${s.buyCount}. SELL-сигналы: ${s.sellSignals}. Удержано от продажи в минус: ${s.skippedSellLoss || 0}.`;
    }
    return;
  }
  const closedPositiveButTotalNegative = s.closedPnl > 0 && s.pnl < 0;
  if (closedPositiveButTotalNegative) {
    box.classList.add('warn');
    verdict.textContent='Закрытые сделки в плюс, но итоговая equity в минус';
    note.textContent=`Закрытый PnL: +${fmt(s.closedPnl)} USDT. Открытый PnL: ${s.openUnrealized>=0?'+':''}${fmt(s.openUnrealized)} USDT. Итог: ${s.pnl>=0?'+':''}${fmt(s.pnl)} USDT. Минус появляется из-за открытой позиции/текущей оценки, а не из-за закрытой продажи в минус.`;
  } else if(s.pnl>0 && s.profitFactor>=1){box.classList.add('good'); verdict.textContent='Результат положительный';
    note.textContent=`Закрытый PnL: ${s.closedPnl>=0?'+':''}${fmt(s.closedPnl)} USDT. Сделок: ${s.trades}. SELL в минус удержано: ${s.skippedSellLoss || 0}.`;
  }
  else if(s.openPosition && s.skippedSellLoss){box.classList.add('warn'); verdict.textContent='Позиция удерживается Profit Guard';
    note.textContent=`Закрытый PnL: ${s.closedPnl>=0?'+':''}${fmt(s.closedPnl)} USDT. Открытый PnL: ${s.openUnrealized>=0?'+':''}${fmt(s.openUnrealized)} USDT. SELL в минус удержано: ${s.skippedSellLoss || 0}.`;
  }
  else {box.classList.add('bad'); verdict.textContent='Результат отрицательный';
    note.textContent=(s.stoppedReason?`Бектест остановлен: ${s.stoppedReason} `:'') + `Закрытый PnL: ${s.closedPnl>=0?'+':''}${fmt(s.closedPnl)} USDT. Сделок: ${s.trades}. BUY: ${s.buyCount}. SELL: ${s.sellCount}.`;
  }
}
...
function renderTabs(bt,inp){
  const allTrades = bt.trades || [];
  const shownTrades = allTrades.slice(-MAX_TABLE_ROWS);
  const omitted = Math.max(0, allTrades.length - shownTrades.length);
  const note = omitted ? `<div class="manualNote">Показаны последние ${shownTrades.length} сделок из ${allTrades.length}. Полный список можно выгрузить кнопкой CSV справа сверху.</div>` : '';
  const tradesHtml = shownTrades.length ? `${note}<table><thead><tr><th>#</th><th>Вход</th><th>Выход</th><th>Цена входа</th><th>Цена выхода</th><th>Размер</th><th>PnL</th><th>Комментарий</th></tr></thead><tbody>${shownTrades.map(t=>`<tr><td>${t.id}</td><td>${dt(t.entryTime)}</td><td>${dt(t.exitTime)}</td><td>${fmtSmart(t.entry)}</td><td>${fmtSmart(t.exit)}</td><td>${fmt(t.size)} USDT</td><td class="${t.pnl>=0?'green':'red'}">${t.pnl>=0?'+':''}${fmt(t.pnl)} USDT</td><td>${t.entryReason}<br>${t.reason}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">Закрытых сделок нет.</div>';
  $('tabTrades').innerHTML=tradesHtml;
  ...
  const manualRows=allTrades.slice(0,6).map(t=>`<tr><td>#${t.id}</td><td>${dt(t.entryTime)}</td><td>BUY dist = ${fmt(t.buyDist,3)}%<br>Open 4H: ${fmtSmart(t.entry4hOpen)} · Low 4H: ${fmtSmart(t.entry4hLow)}</td><td>${dt(t.exitTime)}</td><td>SELL dist = ${fmt(t.sellDist,3)}%<br>Open 4H: ${fmtSmart(t.exit4hOpen)} · High 4H: ${fmtSmart(t.exit4hHigh)}</td></tr>`).join('');
  $('tabQuality').innerHTML = `<div class="manualNote">Ручная сверка показывает, по каким данным можно проверить несколько сделок глазами: цена, время, формула BUY/SELL, текущая 4H-свеча из 1m и факт закрытия только в плюс.</div>${manualRows?`<table><thead><tr><th>Сделка</th><th>BUY время</th><th>BUY сверка</th><th>SELL время</th><th>SELL сверка</th></tr></thead><tbody>${manualRows}</tbody></table>`:'<div class="empty">Нет сделок для ручной сверки.</div>'}`;

  const q=state.quality;
  const warnings=[...(q?.warnings||[]),...(bt.warnings||[])];
  $('tabDetails').innerHTML = `<div class="detailGrid"><div class="detailCard"><span>Логика</span><b>ТЗ + Profit Guard</b></div><div class="detailCard"><span>1m / 4H</span><b>${fmtInt(bt.candles1m.length)} / ${fmtInt(bt.candles.length)}</b></div><div class="detailCard"><span>SELL удержано</span><b>${fmtInt(bt.summary.skippedSellLoss || 0)}</b></div><div class="detailCard"><span>Позиция</span><b>${bt.summary.openPosition?'Открыта':'Закрыта'}</b></div><div class="detailCard"><span>Данные</span><b>${q?fmt(q.score,0)+'/100':'—'}</b></div><div class="detailCard"><span>Пропуски</span><b>${q?q.gaps:'—'}</b></div><div class="detailCard"><span>Закрытый PnL</span><b>${bt.summary.closedPnl>=0?'+':''}${fmt(bt.summary.closedPnl)} USDT</b></div><div class="detailCard"><span>Открытый PnL</span><b>${bt.summary.openPosition ? ((bt.summary.openUnrealized>=0?'+':'')+fmt(bt.summary.openUnrealized)+' USDT') : '—'}</b></div><div class="detailCard"><span>Комиссии</span><b>${fmt(bt.summary.totalFees||0)} USDT</b></div><div class="detailCard"><span>Spread/slippage</span><b>${fmt(bt.summary.estimatedSpreadSlipCost||0)} USDT</b></div><div class="detailCard"><span>Баланс нужен</span><b>${fmt(bt.summary.requiredCash)} USDT</b></div><div class="detailCard"><span>Остановлен</span><b>${bt.summary.stoppedReason ? 'Да' : 'Нет'}</b></div></div>${warnings.length?`<div class="manualNote"><b>Предупреждения:</b><br>${warnings.map(w=>'• '+w).join('<br>')}</div>`:''}`;

  const pro = bt.professional || {};
  const wf = pro.wf || { inSample:basicTradeStats([]), outOfSample:basicTradeStats([]), splitTime:inp.from };
  const model = pro.model || {};
  const tests = pro.unitScenarios || [];
  $('tabPro').innerHTML = `<div class="proHero"><b>Профессиональные слои исполнения</b><span>Этот блок не рисует фантазию: исторического L2-стакана в 1m-свечах нет, поэтому стакан, spread, ликвидность, задержка и частичное исполнение моделируются консервативно. Для настоящего L2 нужен отдельный архив стакана.</span></div><div class="detailGrid proGrid"><div class="detailCard"><span>Bid/Ask spread</span><b>${fmt(model.spreadPct,3)}%</b></div><div class="detailCard"><span>Ликвидность</span><b>${fmt(model.liquidityPct,2)}% объёма</b></div><div class="detailCard"><span>Частичные входы</span><b>${fmtInt(pro.partialFills||0)}</b></div><div class="detailCard"><span>Частичные выходы</span><b>${fmtInt(pro.partialExits||0)}</b></div><div class="detailCard"><span>Блоки по ликвидности</span><b>${fmtInt(pro.liquidityBlocks||0)}</b></div><div class="detailCard"><span>Задержка API</span><b>${fmtInt(model.latencyMs||0)} мс</b></div><div class="detailCard"><span>Min order</span><b>${fmt(model.minOrderUsd||0)} USDT</b></div><div class="detailCard"><span>Объёмное скольжение</span><b>${fmt(model.volumeSlippagePct||0,3)}%</b></div></div><div class="proSplit"><div><b>Walk-forward / train</b><span>до ${dt(wf.splitTime,true)}</span><strong>${wf.inSample.pnl>=0?'+':''}${fmt(wf.inSample.pnl)} USDT</strong><small>${wf.inSample.count} сделок · win ${fmt(wf.inSample.winRate)}%</small></div><div><b>Out-of-sample</b><span>после ${dt(wf.splitTime,true)}</span><strong>${wf.outOfSample.pnl>=0?'+':''}${fmt(wf.outOfSample.pnl)} USDT</strong><small>${wf.outOfSample.count} сделок · win ${fmt(wf.outOfSample.winRate)}%</small></div></div><div class="checkGrid">${tests.map(t=>`<div class="checkCard"><b class="${t.ok?'okText':'badText'}">${t.ok?'✓':'✗'} ${t.name}</b><span>${t.note}</span></div>`).join('')}</div>`;
}
...
function exportTrades(){const bt=state.bt;if(!bt||!bt.trades||!bt.trades.length){alert('Сначала запустите бектест. После расчёта кнопка CSV скачает полный журнал сделок.');return;} const header=['id','buy_time','sell_time','entry','exit','size_usdt','qty','pnl_usdt','pnl_pct','buy_distance_pct','sell_distance_pct','fees','spread_pct','latency_ms','volume_slip_pct','entry_reason','exit_reason']; const lines=[header.map(x=>`"${x}"`).join(',')]; for(const t of bt.trades){const row=[t.id,new Date(t.entryTime).toISOString(),new Date(t.exitTime).toISOString(),t.entry,t.exit,t.size,t.qty,t.pnl,t.pnlPct,t.buyDist,t.sellDist,t.fees||0,t.spreadPct||'',t.latencyMs||0,t.volumeSlipPct||0,t.entryReason,t.reason]; lines.push(row.map(x=>`"${String(x).replace(/"/g,'""')}"`).join(','));} downloadText('strict_tz_trades.csv',lines.join('\n'),'text/csv;charset=utf-8');}
...
function initChartInteractions(){
  ...
  canvas.addEventListener('wheel',e=>{
    if(!state.bt?.candles?.length)return;
    e.preventDefault();
    const plot=state.chart.plot, pt=canvasPoint(e);
    if(!plot)return;
    const anchor=chartIndexFromX(pt.x,plot);
    const span=Math.max(8,(state.chart.viewEnd??state.bt.candles.length-1)-(state.chart.viewStart??0)+1);
    const factor=e.deltaY<0?.78:1.28;
    let newSpan=clamp(Math.round(span*factor),8,state.bt.candles.length);
    const ratio=span>1?(anchor-(state.chart.viewStart??0))/span:.5;
    let start=Math.round(anchor-newSpan*ratio), end=start+newSpan-1;
    if(start<0){end-=start;start=0}
    if(end>=state.bt.candles.length){start-=end-state.bt.candles.length+1;end=state.bt.candles.length-1}
    state.chart.viewStart=clamp(start,0,state.bt.candles.length-1);
    state.chart.viewEnd=clamp(end,state.chart.viewStart,state.bt.candles.length-1);
    drawChart();
  },{passive:false});

  canvas.addEventListener('mousedown',e=>{
    if(!state.chart.plot || !state.bt?.candles?.length) return;
    if(e.button === 2) return;
    const pt=canvasPoint(e);
    const idx=chartIndexFromX(pt.x);
    const isPan = e.button === 1 || e.ctrlKey || e.metaKey || e.altKey;
    state.chart.drag={type:isPan?'pan':'select',x:pt.x,y:pt.y,startIdx:idx,startPrice:chartPriceFromY(pt.y),currentPrice:chartPriceFromY(pt.y),viewStart:state.chart.viewStart,viewEnd:state.chart.viewEnd,currentIdx:idx,moved:false};
  });
  ...
  window.addEventListener('mousemove',e=>{
    ...
    const pt=canvasPoint(e);
    if(state.chart.drag){
      const dx=pt.x-state.chart.drag.x;
      const dy=pt.y-state.chart.drag.y;
      if(Math.hypot(dx,dy)>5*DPR()) state.chart.drag.moved=true;
      if(state.chart.drag.type==='pan'&&state.bt?.candles?.length){
        const span=state.chart.drag.viewEnd-state.chart.drag.viewStart;
        const delta=Math.round(-dx/Math.max(1,plot.plotW)*Math.max(1,span));
        let s=state.chart.drag.viewStart+delta,e2=state.chart.drag.viewEnd+delta;
        if(s<0){e2-=s;s=0}
        if(e2>=state.bt.candles.length){s-=e2-state.bt.candles.length+1;e2=state.bt.candles.length-1}
        state.chart.viewStart=clamp(s,0,state.bt.candles.length-1);
        state.chart.viewEnd=clamp(e2,state.chart.viewStart,state.bt.candles.length-1);
      } else if(state.chart.drag.type==='select'){
        state.chart.drag.currentIdx=chartIndexFromX(pt.x);
        state.chart.drag.currentPrice=chartPriceFromY(pt.y); state.chart.selection={start:state.chart.drag.startIdx,end:state.chart.drag.currentIdx,priceA:state.chart.drag.startPrice,priceB:state.chart.drag.currentPrice};
      }
      drawChart();
      return;
    }
    if(pt.x<plot.pad.l||pt.x>plot.W-plot.pad.r||pt.y<plot.pad.t||pt.y>plot.volBase){
      state.chart.hover=null;state.chart.hoverMarker=null;drawChart();return;
    }
    state.chart.hoverMarker=nearestMarker(pt);
    state.chart.hover={index:chartIndexFromX(pt.x),x:pt.x,y:pt.y,price:chartPriceFromY(pt.y)};
    drawChart();
  });

  window.addEventListener('mouseup',()=>{
    if(state.chart.drag?.type==='select'){
      if(state.chart.drag.moved && createSavedArea(state.chart.drag.startIdx,state.chart.drag.currentIdx,state.chart.drag.startPrice,state.chart.drag.currentPrice)) {
        state.chart.selection=null;
      }
    }
