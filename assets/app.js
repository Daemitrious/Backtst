...
function runStrict(candles1m, inp) {
  ...
  const requiredCash = inp.positionSize * (1 + fee);
  ...
  if (inp.positionSize > inp.startBalance) warnings.push('Размер сделки больше стартового баланса: новые BUY будут невозможны без достаточного баланса.');
  ...
  for (let i = 0; i < candles1m.length; i++) {
    ...
    if (!pos) {
      ...
      if (buySignal) {
        ...
        buyCount += 1;
        addLog({ t: c.t, text: `BUY ${fmtSmart(entry)} · ${pos.reason}` });
      }
    } else {
      ...
      if (sellSignal) {
        ...
        addLog({ t: c.t, text: `SELL ${fmtSmart(exit)} · PnL ${pnl>=0?'+':''}${fmt(pnl)} USDT · ${trade.reason}` });
      }
    }
  ...
  return {
    ...
    summary: { start: inp.startBalance, final, pnl: final - inp.startBalance, ret, annual, trades: trades.length, buyCount, sellCount, wins: winsCount, losses: lossesCount, winRate: trades.length ? winsCount / trades.length * 100 : 0, profitFactor: pf, maxDd, maxDdUsd, ignoredBuy, ignoredSell, openPosition: !!pos, stoppedReason }
  };
}
