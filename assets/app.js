...
const state = {
  candles1m: [], bt: null, sourceLabel: '', sourceKind: '', dataKey: '', runId: 0,
...
function getInputs() {
  return {
    ...
    tf: '240',
    from: dateMs($('dateFrom').value, false),
...
async function fetchJson(url) { const ctrl = new AbortController(); const tm = setTimeout(()=>ctrl.abort(), 7000); try { const r = await fetch(url, { cache:'no-store', signal: ctrl.signal }); if (!r.ok) throw new Error(`HTTP ${r.status}`); return await r.json(); } finally { clearTimeout(tm); } }
async function fetchBybit1m(inp) {
  const categories = ['spot', 'linear'];
  let lastErr = null;
  for (const category of categories) {
    try {
      const all = []; let start = inp.from; let pages = 0; const maxPages = 36; const limit = 1000;
      ...
      if (out.length >= 30) { state.sourceLabel = `Bybit Public API · ${category} · 1m · ${out.length} свечей${pages>1?` · ${pages} запросов`:''}`; state.sourceKind = 'real'; return out; }
...
async function fetchBinance1m(inp) {
  ...
  while (start <= inp.to && pages < maxPages) {
    ...
    start = +rows[rows.length - 1][0] + min1; pages += 1; await sleep(60);
  }
  const out = clean1m(all, inp.from, inp.to);
...
async function load1m(inp) {
  ...
  if (inp.source === 'bybit') { try { return await fetchBybit1m(inp); } catch (e) { const out = demo1m(inp); state.sourceLabel += ` · Bybit недоступен (${e.message})`; return out; } }
  if (inp.source === 'binance') { try { return await fetchBinance1m(inp); } catch (e) { const out = demo1m(inp); state.sourceLabel += ` · Binance недоступен (${e.message})`; return out; } }
  try { return await fetchBybit1m(inp); } catch (e1) { try { return await fetchBinance1m(inp); } catch (e2) { const out = demo1m(inp); state.sourceLabel += ` · API недоступен (${e1.message}; ${e2.message})`; return out; } }
}
...
function drawChart() {
  ...
  const maxVol=Math.max(...candles.map(c=>c.volume),1); const cw=clamp(plotW/candles.length*.64,2*dpr,16*dpr);
...
async function recalculate(forceLoad=false){const runId=++state.runId; const inp=getInputs(); $('csvBox')?.classList.toggle('hidden', inp.source!=='csv'); const key=dataKey(inp); try{ if(forceLoad||key!==state.dataKey||!state.candles1m.length){ state.candles1m=await load1m(inp); state.dataKey=key; resetView(); } state.quality=analyze1mQuality(state.candles1m); const bt=runStrict(state.candles1m,inp); if(runId!==state.runId)return; state.bt=bt; render(inp,bt);}catch(e){$('resultVerdict').textContent='Ошибка расчёта'; $('resultNarrative').textContent=e.message; console.error(e);} }
